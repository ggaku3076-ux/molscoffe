...
export default function MenuSection() {
  ...
  const menuItems: MenuItem[] = [
    // Main Course
    {
      ...
      price: 33000,
      description: "Nasi hangat dengan bebek goreng bumbu rempah melimpah khas Mols Coffe, lalapan segar, dan sambal mantap.",
      isPopular: true,
    },
    {
      id: "mc-2",
    ...
    },
    {
      ...
      category: "main-course",
      price: 28000,
      description: "Rice bowl beef slice manis gurih dengan topping telur mata sapi ala Mols Coffe.",
    },
    {
      id: "mc-6",
    ...
    {
      id: "mc-12",
      name: "Nasi Goreng Mols",
      category: "main-course",
      price: 25000,
      description: "Nasi goreng merah ala Mols Coffe lengkap dengan telur mata sapi dan kerupuk.",
    },
    {
      id: "mc-13",
    ...
    {
      id: "sn-1",
      name: "Mols French Fries",
      category: "snacks-new",
      price: 23000,
      description: "Kentang goreng renyah bumbu spesial Mols Coffe disajikan dengan cocolan saus pilihan.",
    },
    {
      id: "sn-2",
      name: "Mix Plater A",
    ...
    },
    {
      ...
      category: "snacks-new",
      price: 33000,
      description: "Porsi bebek goreng renyah gurih dengan sambal bajak pedas nikmat khas Surabaya.",
    },
    {
      id: "sn-6",
  ...
  const handleWhatsAppSubmit = (e: React.FormEvent) => {
    ...
    let message = `Halo Mols Coffe Surabaya, saya ingin memesan menu takeaway atas nama *${checkoutName}*:\n\n`;
    cart.forEach((item) => {
    ...
    window.open(`https://wa.me/628123530547?text=${encoded}`, "_blank");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between">
                  ...
                  <button
                    ...
                    className={`rounded px-4 py-2 text-xs font-semibold tracking-wider transition-colors uppercase ${
                      selectedCategory === cat.id
                        ...
                        : "bg-brand-brown-bg text-brand-brown border border-brand-brown/15 hover:bg-brand-brown/10"
                    }`}
                  >
                    {cat.name}
                  </button>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full sm:w-60 pl-9 pr-4 py-2 text-sm border border-brand-dark/10 rounded focus:border-brand-brown focus:ring-0 focus:outline-none bg-white text-brand-dark"
                />
              </div>
            </div>
            ...
            {filteredMenuItems.length === 0 ? (
              <div className="text-center py-12 border border-dashed border-brand-brown/20 rounded-xl bg-brand-brown-bg">
                <ListFilter className="h-8 w-8 text-brand-brown mx-auto mb-3" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredMenuItems.map((item) => (
                  <div 
                    key={item.id} 
                    className="p-6 rounded-xl border border-brand-dark/5 bg-white hover:border-brand-brown transition-all duration-200 flex flex-col justify-between"
                  >
                    <div className="flex flex-col gap-2">
                      <div className="flex justify-between items-start gap-4">
                        <h3 className="font-bold text-brand-dark text-lg">{item.name}</h3>
                        <span className="font-mono font-bold text-brand-brown shrink-0">
                          {formatPrice(item.price)}
                        </span>
                      </div>
                      
                      {item.isPopular && (
                        <span className="inline-flex max-w-fit rounded bg-brand-brown/10 px-2 py-0.5 text-[10px] font-bold text-brand-brown uppercase">
                          Menu Andalan
                        </span>
                      )}
                    ...
                    <button
                      ...
                      className="mt-4 flex items-center justify-center gap-2 rounded border border-brand-brown px-4 py-2 text-xs font-semibold text-brand-brown hover:bg-brand-brown hover:text-white transition-colors duration-200"
          ...
          <div className="lg:col-span-4 sticky top-24">
            <div className="border border-brand-brown/30 bg-brand-brown-bg rounded-2xl p-6 flex flex-col gap-6 shadow-sm">
              <div className="flex items-center justify-between border-b border-brand-brown/20 pb-4">
                <div className="flex items-center gap-2">
                  <ShoppingBag className="h-5 w-5 text-brand-brown" aria-hidden="true" />
                  <h3 className="font-extrabold text-brand-dark text-lg">Simulasi Pesanan</h3>
                </div>
                <span className="font-mono text-xs font-bold bg-brand-brown text-white rounded-full px-2 py-0.5">
                  ...
                  {cart.map((item) => (
                    <div key={item.id} className="flex items-start justify-between gap-3 text-left">
                      <div className="flex flex-col">
                        ...
                        <span className="font-mono text-xs text-brand-brown font-semibold mt-1">
                          {formatPrice(item.price)}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <button
                          ...
                          className="p-1 rounded bg-white border border-brand-brown/20 text-brand-dark hover:border-brand-brown"
                        ...
                        <button
                          ...
                          className="p-1.5 text-brand-dark/40 hover:text-brand-brown"
              ...
              {/* Total Calculation */}
              <div className="border-t border-brand-brown/25 pt-4 flex flex-col gap-4">
                  ...
                  <form onSubmit={handleWhatsAppSubmit} className="flex flex-col gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-2 text-xs border border-brand-brown/30 rounded focus:border-brand-brown focus:ring-0 focus:outline-none bg-white text-brand-dark"
                      />
                    </div>
                    <button
                      type="submit"
                      disabled={isOrdered}
                      className="w-full rounded bg-brand-brown py-3 text-center text-xs font-bold tracking-wider text-white uppercase hover:bg-brand-brown-light transition-colors duration-200 flex items-center justify-center gap-2"
                    >
                      {isOrdered ? (
                        <>
