...
export default function Navbar() {
  ...
  const headerBgClass = isHome 
    ...
    : "absolute top-0 left-0 w-full z-50 bg-brand-brown shadow-md border-b border-white/10";

  const ctaBtnClass = isHome
    ? "bg-brand-brown text-white hover:bg-brand-brown-light"
    : "bg-white text-brand-brown hover:bg-white/95";
    ...
    <>
      <header className={headerBgClass}>
        <div className="mx-auto grid grid-cols-2 md:grid-cols-3 items-center max-w-7xl p-4 md:px-8">
          ...
          <Link 
            ...
            aria-label="Mols Coffe - Kembali ke Beranda"
          >
      ...
      >
        ...
        >
          ...
          {/* Divider */}
          <div className="w-12 h-0.5 bg-brand-brown/20 rounded-full" />
            ...
            return (
              <Link
                ...
                className={`text-base font-semibold tracking-wide transition-all duration-200 py-1.5 ${
                  active ? "text-brand-brown" : "text-brand-dark/70 hover:text-brand-brown"
                }`}
          ...
          <Link
            href="/rsvp"
            onClick={() => setIsOpen(false)}
            className="inline-flex items-center justify-center gap-2 w-full rounded-2xl bg-brand-brown py-3 text-sm font-bold text-white hover:bg-brand-brown-light transition-colors duration-200"
          >
            <span>Shop Now</span>
            <ArrowRight className="h-4 w-4" aria-hidden="true" />
          </Link>
