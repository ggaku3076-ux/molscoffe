...
export default function VenueSection() {
  ...
  const handleRsvpsubmit = (e: React.FormEvent) => {
    ...
    let message = `Halo Mols Coffe Surabaya, saya ingin menanyakan ketersediaan tempat untuk acara privat:\n\n`;
      ...
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-base text-brand-dark/70 mt-4">
            Ingin mengadakan rapat kantor, kelas belajar, ulang tahun, atau reuni? Gunakan kalkulator estimasi area kami untuk menemukan ruangan terbaik di Mols Coffe Surabaya.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <form 
            ...
            className="lg:col-span-5 bg-white p-6 sm:p-8 rounded-2xl border border-brand-brown/10 flex flex-col gap-5 justify-between"
          >
            <h3 className="text-xl font-bold text-brand-dark flex items-center gap-2 border-b border-brand-brown/10 pb-4">
              <Calendar className="h-5 w-5 text-brand-brown" aria-hidden="true" />
            ...
            <div className="flex flex-col gap-1 text-left">
              ...
              <input
                ...
                className="w-full px-4 py-2.5 text-sm border border-brand-dark/10 rounded focus:border-brand-brown focus:ring-0 focus:outline-none bg-white text-brand-dark"
              />
            </div>
            ...
            <div className="flex flex-col gap-2 text-left">
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono text-sm font-bold text-brand-brown flex items-center gap-1">
            ...
            <button
              ...
              className="w-full mt-2 rounded bg-brand-brown py-3.5 text-center text-sm font-bold tracking-wider text-white uppercase hover:bg-brand-brown-light transition-colors duration-200 flex items-center justify-center gap-2"
            >
              {isSubmitted ? (
                <>
          ...
          <div className="lg:col-span-7 flex flex-col justify-between p-6 sm:p-8 bg-white border border-brand-brown/15 rounded-2xl">
            <div className="flex flex-col gap-4">
              <div className="inline-flex max-w-fit items-center gap-1.5 rounded-full border border-brand-brown/20 bg-brand-brown-bg px-3 py-1 text-xs font-bold text-brand-brown">
              ...
              <div className="relative aspect-[16/8] w-full rounded-xl border border-brand-brown/30 bg-brand-brown-bg/40 p-4 flex flex-col justify-between overflow-hidden">
                <div className="absolute inset-0 grid grid-cols-8 grid-rows-3 pointer-events-none opacity-10">
                  {Array.from({ length: 24 }).map((_, i) => (
                    <div key={i} className="border-t border-l border-brand-brown/30"></div>
                  ))}
                </div>
                <span className="font-mono text-[9px] text-brand-brown font-bold uppercase tracking-wider">
                  Layout Blueprint (Skala 1:100)
                </span>
                
                <div className="flex items-center justify-center my-auto flex-col gap-1">
                  <div className="font-mono text-xs font-semibold text-brand-dark/70 border border-brand-brown/40 rounded px-2.5 py-1 bg-white">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-left">
                  {recommendedArea.features.map((feat, i) => (
                    <div key={i} className="flex items-center gap-2 text-sm text-brand-dark/80">
                      <CheckCircle2 className="h-4 w-4 text-brand-brown shrink-0" aria-hidden="true" />
                      <span>{feat}</span>
                    </div>
                  ))}
                </div>
            ...
            <div className="border-t border-brand-dark/5 pt-6 mt-6 text-left">
              <p className="text-xs text-brand-dark/50 leading-normal">
                *Paket sewa area di Mols Coffe Surabaya bersifat fleksibel, dapat berupa sistem sewa per jam atau minimum pembelanjaan makanan & minuman (Minimum Spend). Hubungi admin kami untuk informasi lebih lanjut.
              </p>
            </div>
          </div>
        </div>
