...
export default function Home() {
  const portalCards = [
    ...
    },
    {
      icon: MapPin,
      title: "Lokasi & Kontak",
      description: "Temukan jam operasional dan petunjuk arah Google Maps langsung ke kafe kami di di Wonokromo, Surabaya.",
      href: "/lokasi",
  ...
  return (
    <>
      ...
      <section 
        className="py-16 md:py-24 bg-brand-brown-bg border-t border-brand-brown/10"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {portalCards.map((card, index) => {
              ...
              return (
                <div 
                  key={index}
                  className="bg-white p-8 rounded-2xl border border-brand-brown/10 hover:border-brand-brown transition-all duration-300 flex flex-col justify-between items-start text-left shadow-sm hover:shadow-md"
                >
                  <div className="flex flex-col gap-4">
                    <div className="h-12 w-12 rounded-xl bg-brand-brown-bg border border-brand-brown/20 flex items-center justify-center text-brand-brown shrink-0">
                  ...
                  <Link
                    href={card.href}
                    className="mt-6 inline-flex items-center gap-1 text-sm font-bold text-brand-brown hover:text-brand-brown-light group"
                  >
